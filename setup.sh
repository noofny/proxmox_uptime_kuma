...
# prompts/args
DEFAULT_HOSTNAME='monitor-1'
DEFAULT_PASSWORD='monitoradmin'
...
pct resize "${CONTAINER_ID}" rootfs 20G || fatal "Failed to expand root volume!"
...
wget -qL https://raw.githubusercontent.com/noofny/proxmox_uptime_kuma/master/setup_os.sh
...
# Setup Docker
info "Fetching setup script..."
wget -qL https://raw.githubusercontent.com/noofny/proxmox_uptime_kuma/master/setup_docker.sh
...
# Setup Uptime Kuma
info "Fetching setup script..."
wget -qL https://raw.githubusercontent.com/noofny/proxmox_uptime_kuma/master/setup_uptime_kuma.sh
wget -qL https://raw.githubusercontent.com/noofny/proxmox_uptime_kuma/master/docker-compose.yaml
info "Executing script 'setup_uptime_kuma.sh'..."
cat ./setup_uptime_kuma.sh
pct push "${CONTAINER_ID}" ./setup_uptime_kuma.sh /setup_uptime_kuma.sh -perms 755
pct push "${CONTAINER_ID}" ./docker-compose.yaml /docker-compose.yaml
pct exec "${CONTAINER_ID}" -- bash -c "/setup_uptime_kuma.sh" || fatal "Script execution failed!"
pct reboot "${CONTAINER_ID}"
